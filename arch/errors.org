...
#+BEGIN_SRC go
  // ProcessData instructs the BigStruct it receives to read data from
  ...
  func ProcessData(r *reporting.TimeSheet) (string,error){
    ...
    // probably something that figures out what file to read from
...
better. But we still need to handle these errors; the code calling =ProcessData=
should only get an error if this function completely fails to handle these
errors.
...
#+BEGIN_SRC go
      func (ts *TimeSheet) Process(input io.Reader) (io.Reader, error) {
         ...
         // do some other stuff with output
...
I think this is the hottest take I've ever seen from a Go personality; from Dave Cheney:
...
Some of the things in this section may go away depending on what happens with Go
2.

But do you know what is better than an improved syntax for handling errors? Not
needing to handle errors at all. I’m not saying "remove your error
handling". What I am suggesting is this: change your code so you do not have errors
to handle.
...
I think this is a good example of Russ Cox’s observation that error handling can
obscure the operation of the function. Let’s look at an improved version.
...
Earlier in this presentation we went over examples dealing with opening, writing
and closing files. The error handling is present, but not overwhelming as the
operations can be encapsulated in helpers like =ioutil.ReadFile= and
=ioutil.WriteFile=. However when dealing with low level network protocols it
becomes necessary to build the response directly using I/O primitives the error
handling can become repetitive. Consider this fragment of a HTTP server which is
...
Lastly, I want to mention that you should only handle errors once. Handling an
...
problematic. The following is code that Dave Chaney comes across frequently.
...
I want to dig into this a little further because I don’t see the problems with
...
The problem I see a lot is programmers forgetting to return from an error. As we
...
This brings me to a Go proverb that I want to talk about; don’t just check
errors, handle them gracefully. Can you suggest some problems with the following
piece of code?
...
review. More fundamentally the problem with this code is I cannot tell where the
...
Unfortunately I don't have any great guidelines on how to best use
wrapping... yet.

For now try to only wrap an error when the error that is returned contains /no/
